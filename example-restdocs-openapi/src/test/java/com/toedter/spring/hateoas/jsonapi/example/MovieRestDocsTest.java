...
class MovieRestDocsTest {
                ...
                .andDo(document("movies-get",
                        resource(
                                ResourceSnippetParameters.builder()
                                        ...
                                        .responseFields(
                                                fieldWithPath("jsonapi.version").description("the JSON:API version"),
                                                subsectionWithPath("data").description("an array of movies"),
                                                subsectionWithPath("links").description("links to next, previous, ... pages"),
    ...
    void should_get_movie() throws Exception {
        resultActions = mockMvc.perform(get("/api/movies/{id}", 1).accept(JSON_API_VALUE));

        resultActions
                .andExpect(status().isOk())
                .andDo(document("movie-get",
                        resource(
                                ResourceSnippetParameters.builder()
                                        ...
                                        .responseFields(
                                                ...
                                                fieldWithPath("data.attributes.title").description("the movie's title"),
                                                fieldWithPath("data.attributes.year").description("the movie's release year"),
                                                fieldWithPath("data.attributes.imdbId").description("the movie's Internet Movie Database (IMDB) Id"),
                                                fieldWithPath("data.attributes.rating").description("the movie's IMDB rating"),
                                                fieldWithPath("data.attributes.rank").description("the movie's IMDB rank"),
                                                subsectionWithPath("data.relationships.directors").description("the movie's directors"),
                                                subsectionWithPath("links.self").description("the movie's self link"))
                                        .build()
                        )
                ));
    }
